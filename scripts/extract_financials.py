...
def _fetch_chunks_from_knowledge_base(
    ...
    """
    stored_chunks = admin_service.read_chunks_sync(source_file=str(pdf_path))
    if not stored_chunks:
        return []

    pairs = sorted(
        ((chunk.content, chunk.metadata) for chunk in stored_chunks),
...
def extract_one_pdf(
    ...
    chunks = _fetch_chunks_from_knowledge_base(pdf_path, admin_service)
    if not chunks:
