...
from knowledge.ingestion.contracts import ParseRequest, ParseResult, ParsedChunk
from knowledge.ingestion.source import source_identity
...
class LocalPdfParserAdapter:
    ...
    def _parse_sync(request: ParseRequest) -> ParseResult:
        from knowledge.ingestion.infrastructure.parsing.pdf_parser import (
        ...
        for index, document in enumerate(documents or []):
            ...
            chunk_id = str(
                metadata.get("chunk_id") or f"{source_file}::local_{index:06d}"
            )
