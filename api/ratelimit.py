...
def _key_id(bucket_key: str) -> str:
    """
    生成 bucket_key 的不可逆摘要，避免 API Key 或客户端地址出现在 Redis key 中。

    截取 SHA-256 的前 24 个十六进制字符（96 bit），兼顾键长度与碰撞空间。
    """
    if not bucket_key:
        return "unknown"
    return hashlib.sha256(bucket_key.encode("utf-8")).hexdigest()[:24]
