...
**做了什么**:`AgentService.stream_events()` 基于 `astream_events(version="v2")`;SSE 事件 taxonomy:`token` / `usage` / `tool_call` / `tool_result` / `done` / `error`;Queue + Task 架构解耦 producer 与 SSE 输出;手动 deadline(兼容 Python 3.10);客户端断连即取消上游。
