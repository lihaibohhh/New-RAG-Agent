...
def _get_output_dir() -> Path:
    ...
    if output_dir is None:
        raise RuntimeError("Docx Tool 必须由 Runtime 注入 output_dir 后使用")
...
def create_docx_tool(*, output_dir: str | Path):
    """Create a Word adapter bound to Runtime-owned output configuration."""
    configured_output_dir = Path(output_dir)

    @tool(
        "docx_tool",
        description=docx_tool.description,
        args_schema=docx_tool.args_schema,
    )
    def configured_docx_tool(
        title: str = "",
        sections: Any = None,
        filename: str = "",
        mode: str = "timestamp",
        ...
        token = _OUTPUT_DIR.set(configured_output_dir)
        try:
            return docx_tool.func(
                title=title,
                sections=sections,
                filename=filename,
                mode=mode,
    ...
    return configured_docx_tool


__all__ = ["create_docx_tool"]
