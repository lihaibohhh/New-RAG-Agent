...
def _get_output_dir() -> Path:
    ...
    if output_dir is None:
        raise RuntimeError("Markdown Tool 必须由 Runtime 注入 output_dir 后使用")
...
def create_markdown_tool(*, output_dir: str | Path):
    """Create a Markdown adapter bound to Runtime-owned output configuration."""
    configured_output_dir = Path(output_dir)

    @tool(
        "md_tool",
        description=md_tool.description,
        args_schema=md_tool.args_schema,
    )
    def configured_markdown_tool(
        title: str = "",
        sections: Any = None,
        filename: str = "",
        mode: str = "timestamp",
    ...
    return configured_markdown_tool


__all__ = ["create_markdown_tool"]
