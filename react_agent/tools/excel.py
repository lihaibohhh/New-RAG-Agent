...
# 配置环境变量：
#   - OUTPUT_DIR：Excel/Word/Markdown 共用输出目录（默认锚定应用根目录 outputs）
...
async def _make_excel_impl(
    ...
    # ════════════════════ 阶段 4：输出目录准备 ════════════════════
    out_dir = _OUTPUT_DIR.get()
    if out_dir is None:
        raise RuntimeError("Excel Tool 必须由 Runtime 注入 output_dir 后使用")
...
def create_excel_tool(
    *,
    output_dir: str | Path,
    default_mode: str = "timestamp",
    keep_backup: bool = True,
):
    """Create an Excel adapter bound to Runtime-owned output configuration."""
    configured_output_dir = Path(output_dir)

    async def configured_excel(
        filename: str,
        headers: list[str],
        rows: list[list[Any]],
        sheet_name: str = "Sheet1",
