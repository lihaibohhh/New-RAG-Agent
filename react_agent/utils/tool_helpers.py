...
def with_retry(
    ...
    参数：
      ...
      retry_timeouts (bool)：是否重试装饰器产生的超时；计算密集型工具应设为 False，
                            防止底层线程仍运行时重复提交任务
        ...
        async def _call_async(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if is_timeout and not retry_timeouts:
                        break
            ...
            is_timeout = isinstance(last_exc, (TimeoutError, asyncio.TimeoutError))
            if is_timeout:
                code = "TOOL_TIMEOUT"
                ...
                code = "TOOL_FAILED"
                message = f"工具调用失败：{type(last_exc).__name__}: {last_exc}"
