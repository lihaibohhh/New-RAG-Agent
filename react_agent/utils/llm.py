...
职责：
- 解析 AgentContext.model（"provider/model-name"）
...
def _parse_model_ref(model_ref: str) -> Tuple[str, str]:
    ...
    if not model_ref:
        raise ValueError("AgentContext.model 不能为空，例如：'openai/gpt-4.1-mini'")

    if "/" not in model_ref:
        raise ValueError(
            "AgentContext.model 必须是 'provider/model-name' 格式，例如："
    ...
    if not provider or not model_name:
        raise ValueError(
            "AgentContext.model 格式不正确，应该是 'provider/model-name'，例如 'openai/gpt-4.1-mini'"
        )
