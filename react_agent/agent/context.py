...
- Agent 执行参数集中放在 AgentContext：模型、系统提示、搜索参数、步数限制、调试开关、截断策略等
...
from react_agent.agent import prompts
...
class AgentContext:
    ...
    # 会话 ID 是 Agent 每次执行的必要输入；持久化后端和数据库路径不属于
    # Agent 配置，由 ConversationPersistenceConfig 独立管理。
        ...
        for f in fields(self):
            ...
            try:
                ...
                if getattr(self, "debug", False):
                    logger.info(f"[AgentContext] ⚠️ env {env_key}={raw!r} 无法转换为 {tp}，已忽略覆盖")
                continue
