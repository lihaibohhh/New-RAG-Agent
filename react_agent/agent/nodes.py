    ...
    while i < len(msgs):
        ...
        if need_ids:
            ...
            while j < len(msgs) and isinstance(msgs[j], ToolMessage):
                out.append(msgs[j])
                answered.append(msgs[j].tool_call_id)
                j += 1
            for _id in need_ids:
            ...
            "messages": [
                AIMessage(
                    ...
                    content=(
                        ...
                        "因此已停止继续搜索/调用工具。\n"
                        "你可以：\n"
                        "- 提高 AgentContext.recursion_limit；或\n"
                        "- 进一步澄清问题/提供更多上下文。"
                    ),
                )
            ],
...
async def dynamic_tool_node(state: State, config: RunnableConfig, runtime: Runtime[AgentDependencies]) -> Dict[str, Any]:
    """
    动态权限边界：避免执行层挂载未经过滤的全量工具，否则即使 call_model
    过滤了工具，执行层面仍能调用被禁止的工具，权限控制形同虚设。

    本节点在运行时根据 AgentContext 动态构建只含可用工具的 ToolNode，确保过滤真正生效。
